...
export default function Home() {
  ...
  // --- CONFIGURAÇÕES ---
  const CHAVE_PIX = "11981102244"; // Celular sem formatação para copiar fácil
  const CHAVE_PIX_FORMATADA = "(11) 98110-2244"; // Para exibir bonito
  
  const DATA_SORTEIO = "25/12/2024"; 
  ...
  // --- AQUI MUDOU: Só reserva e joga pra tela de pagamento ---
  const finalizarCompra = async () => {
    ...
    if (success) {
      ...
      setSelecionados([]);
      setVendoMeusPedidos(true); // Joga para a tela de pagamento
      toast.success("Números reservados! Agora faça o PIX.");
    }
  };

  // --- NOVA FUNÇÃO: Gera a mensagem completa com o comprovante ---
  const enviarComprovante = () => {
    if (pendentes.length === 0) return;

    const listaNumeros = pendentes.map(p => p.numero).join(", ");
    const total = (pendentes.length * 5).toFixed(2);
    const primeiroNome = nome.split(' ')[0];

    const msg = `🧾 *ENVIAR COMPROVANTE*\n\n👤 *Nome:* ${primeiroNome}\n🎟 *Números:* ${listaNumeros}\n💰 *Total:* R$ ${total}\n\nSegue o comprovante do PIX abaixo: 👇`;
    
    // Abre o WhatsApp direto com essa mensagem
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl w-full max-w-xs p-6 shadow-2xl relative">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3"><div className="w-4 h-4 rounded bg-amber-300 border border-amber-400 mt-1 shrink-0"></div><div><p className="font-bold text-sm text-amber-700">Reservado</p><p className="text-xs text-slate-500">Aguardando pagamento. Se não pagar em 3 dias, libera.</p></div></div>
        ...
        {vendoMeusPedidos ? (
          <div className="flex-1 p-6 animate-in fade-in slide-in-from-bottom-4 bg-slate-50/50 overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 shadow-sm relative overflow-hidden">
                ...
                {NUMERO_GANHADOR !== null ? (
                  <div className="relative z-10">
                    <p className="text-xs text-purple-600 mb-2">Número sorteado:</p><div className="text-3xl font-black text-purple-900 mb-3">#{NUMERO_GANHADOR}</div>
              ...
              {/* ÁREA DE PAGAMENTO (Se tiver pendências) */}
              {pendentes.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm space-y-4 animate-in slide-in-from-bottom-2">
                  <div className="bg-amber-50 p-4 rounded-lg border border-amber-100 text-center">
                    ...
                    <button 
                      onClick={copiarPix} 
                      className="w-full bg-white border border-amber-200 hover:border-amber-300 rounded-lg p-3 flex items-center justify-between gap-3 group transition-all"
                    >
                      <div className="text-left">
                        <span className="text-[10px] font-bold text-slate-400 block uppercase">Chave Celular</span>
                        <code className="text-sm font-bold text-slate-700 group-hover:text-blue-600 transition-colors">{CHAVE_PIX_FORMATADA}</code>
                      </div>
                      <Copy size={18} className="text-amber-400 group-hover:text-blue-500" />
                    </button>

                    <p className="text-xs font-bold text-amber-700 uppercase mt-4 mb-2">2. Envie o Comprovante</p>
                    
                    {/* Botão Único: Manda mensagem completa */}
                    <button 
                      onClick={enviarComprovante}
                      className="w-full h-10 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-lg font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all"
                    >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">{pendentes.map(n => <span key={n.numero} className="px-2 py-1 bg-amber-50 border border-amber-100 text-amber-700 rounded text-xs font-bold">#{n.numero}</span>)}</div>
                  </div>
                </div>
              )}
              ...
              {confirmados.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-emerald-100 shadow-sm"><div className="flex items-center gap-2 mb-3 text-emerald-600"><Check size={16} strokeWidth={3} /><span className="text-xs font-bold uppercase tracking-wider">Pagamento Confirmado</span></div><div className="flex flex-wrap gap-1.5">{confirmados.map(n => <span key={n.numero} className="px-2 py-1 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded text-xs font-bold">#{n.numero}</span>)}</div></div>
              )}
            </div>
